...
def home(request):
    ...
    produits = Product.objects.order_by("-create_at")[:10]
...
def contact(request):
    if request.method == 'POST':
        ...
        try:
            send_email = EmailMessage(subject_formatted, message, email, to=['logic01pro@gmail.com'])
            send_email.send()
            messages.success(request, 'Votre email a ete envoyer avec success. Nous serons ravi de vous lire.')
